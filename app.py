    ...
    if df is not None:
        ...
        eda_choise = st.selectbox(label="Pick the operation you want",
                                  options=['Show shape', 'Show data type', 'Show missing values', 'Description',
                                           ...
                                           'Show Value Counts', 'Show Unique Values', 'Show pandas profiling'],
                                  index=None)
        if eda_choise == None:
            ...
            # st.write("Data type of the Columns :", df.dtypes)
        ...
        elif eda_choise == 'Show pandas profiling':
...
with st.expander("Data Cleaning"):
    if df is not None:
        ...
        Handling_missing_data = st.checkbox("Handling missing data")
        ...
        if Handling_missing_data:
            Handling_missing_data_operation = st.selectbox('Please pick the operation you want',
                                                           ['', 'Drop Nan values', 'Fill missing data'])
            if 'Drop Nan values' in Handling_missing_data_operation:
                drop_na0 = st.checkbox("Drop all rows with Nan values")
                drop_na1 = st.checkbox("Drop all columns with Nan values")
            if 'Fill missing data' in Handling_missing_data_operation:
                ...
                Filling_options = st.multiselect("Select filling operation", options=(
                    "Mean", "Median", "Most frequent", "Random Imputation"))

                if "Mean" in Filling_options:
                    columns_for_Mean_imputation = st.multiselect(
                        "Please pick the columns you want to fill(Mean imputation)",
                        numeric_columns_with_empty_values)

                if "Median" in Filling_options:
                    columns_for_Median_imputation = st.multiselect(
                        "Please pick the columns you want to fill(Median imputation)",
                        numeric_columns_with_empty_values)

                if "Most frequent" in Filling_options:
                    columns_for_Most_frequent_imputation = st.multiselect(
                        "Please pick the columns you want to fill(Most frequent imputation)",
                        columns_with_empty_values)

                if "Random Imputation" in Filling_options:
                    columns_for_Random_imputation = st.multiselect(
                        "Please pick the columns you want to fill(Random imputation)",
                        columns_with_empty_values)
            ...
            if Handling_missing_data:
                ...
                if 'Fill missing data' in Handling_missing_data_operation:
                    if "Mean" in Filling_options:
                        ...
                        st.success('Selected columns filled(with mean) successfully')

                    if "Median" in Filling_options:
                        imputer = SimpleImputer(strategy='median')
                        df[columns_for_Median_imputation] = np.round(
                            imputer.fit_transform(df[columns_for_Median_imputation]), 1)
                        st.success('Selected columns filled(with median imputation) successfully')

                    if "Most frequent" in Filling_options:
                        imputer = SimpleImputer(strategy='most_frequent')
                        df[columns_for_Most_frequent_imputation] = imputer.fit_transform(
                            df[columns_for_Most_frequent_imputation])
                        st.success('Selected columns filled(with most frequent imputation) successfully')

                    if "Random Imputation" in Filling_options:
                        df = random_imputation(df, columns_for_Random_imputation)
                        st.success('Selected columns filled(with most random imputation) successfully')
    ...
    if df is not None:
        ...
        if st.button("Apply data preprocessing"):
            if Scale:
                ...
                st.success('Selected columns has been successfully scaled')
